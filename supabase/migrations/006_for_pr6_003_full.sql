-- ═══════════════════════════════════════════════════════════════
-- FOR.PR6.003 — Colonnes manquantes pour le traitement complet
-- ═══════════════════════════════════════════════════════════════
...
-- Table de détail des produits pour le template FOR.PR6.003
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'tdp_own_exploitation'
  ) THEN
    CREATE POLICY "tdp_own_exploitation" ON treatment_detail_products
      USING (treatment_id IN (SELECT id FROM treatments));
  END IF;
END
$$;
